...
class EstimacionMultipleEscalatoria(models.Model):
    ...
    sub_total_esc = fields.Float('Subtotal de escalatoria Padre', related="estimacion.estimacion_subtotal")
...
class Estimaciones(models.Model):
    ...
    a_fin = fields.Float(string="Avance Financiero", compute="avanceFinanciero")
        ...
        for i in search:
            if not self.idobra:
                ...
                if int(self.idobra) < int(i.idobra):
                    ...
                    acum += i.estimado
                    acum_anti += i.amort_anticipo
                print(acum_anti, ' ACUMULADO ANTICIPO ALVLAVLAVLAVLALV')
                self.acum_anterior = acum
                self.acum_total = acum + self.estimado
                self.saldo_contrato = self.monto_contrato - self.acum_total
    ...
    def B_fechas_programa(self):
        ...
        _search_cove = self.env['proceso.convenios_modificado'].search_count([("nombre_contrato", "=", self.obra.nombre_contrato), ("tipo_convenio", "=", 'PL' or 'BOTH')])
    ...
    def B_ff_programa(self):
        ...
        _search_cove = self.env['proceso.convenios_modificado'].search_count(
            [("contrato.id", "=", self.obra.id), ("tipo_convenio", "=", 'PL' or 'BOTH')])

        b_convenio = self.env['proceso.convenios_modificado'].search([('contrato.id', '=', self.obra.id)])
        if _search_cove > 0:
            for i in b_convenio:
                if i.tipo_convenio == 'PL' or i.tipo_convenio == 'BOTH':
                    fecha_prog = datetime.strptime(str(i.plazo_fecha_termino), date_format).date()
    ...
    '''@api.one
    @api.depends('conceptos_partidas', 'si_aplica')
    ...
    def conceptosEjecutados(self):
        ...
        b_est = self.env['control.estimaciones'].search([('obra.id', '=', self.obra.id)])
        c_est = self.env['control.estimaciones'].search_count([('obra.id', '=', self.obra.id)])
        b_concepto = self.env['control.detalle_conceptos'].search([('id_partida.id', '=', self.obra.id)])
        ...
        if int(c_est) == 0:
            ...
            est_ant_acum = 0
            est_ant = 0
            for x in b_est[int(c_est)-1].conceptos_partidas:

                '''if x.estimacion == 0:
                    est_ant = 0
                else:
                    est_ant = float(x.estimacion)'''

                if int(self.idobra) == 0 or not self.idobra:
                    est_ant = 0
                else:
                    estimacion = int(self.idobra) - 1
                    '''busqueda = self.env['control.detalle_conceptos'].search([('num_est', '=', estimacion),('id_partida.id', '=', self.obra.id)
                                                                            ,('clave_linea', '=', self.clave_linea),('concepto', '=', self.concepto),
                                                                            ('precio_unitario', '=', self.precio_unitario),('cantidad', '=', self.cantidad)])'''

                    busqueda2 = self.env['control.detalle_conceptos'].search([('num_est', '<', int(self.idobra)),('id_partida.id', '=', self.obra.id)
    ...
    @api.multi
    def unlink(self):
        self.conceptos_partidas.unlink()
        self.deducciones.unlink()
        ...
        if not self.estimado:
            ...
            for i in sorted(b_programa.programa_contratos):
                ...
                # CON UNA FORMULA PARA CALCULAR EL MONTO ACTUAL HASTA LA FECHA DE TERMINO DE LA ESTIMACION
                    ...
                    # SON MESES DIFERENTES
                        ...
                        if fecha_terminoest_y_m == fecha_terminop_y_m:
                            ...
                            # SI LA FECHA DE TERMINO DE LA ESTIMACION ES MENOR A LA FECHA DEL TERMINO DEL PROGRAMA
                                ...
                                print(formula, ' formula x')
                                print(ultimo_monto, ' monto')
                                print(acum, ' acum')
                                ...
                                self.diasrealesrelacionqweb = self.montorealqweb / self.montodiario_programadoqweb
                                # DIAS DE DESFASAMIENTO
                                print(self.dias_transcurridosqweb)
                                print(self.diasrealesrelacionqweb)
                                if self.dias_transcurridosqweb <= self.diasrealesrelacionqweb:
                                    print(' KSDK OROEORE LZDSD x1')
                                    self.dias_desfasamiento = 0
                                else:
                                    print(' KSDK OROEORE LZDSD x2')
                                ...
                                self.porc_total_retqweb = self.retencion * self.dias_desfasamientoqweb
                                self.total_ret_estqweb = (self.monto_atrasoqweb * self.porc_total_retqweb) / 100
                                print(self.retenido_anteriormenteqweb, ' RET ANTERIOR')
                                print(self.total_ret_estqweb, ' RET total')
                                if self.retenido_anteriormenteqweb == 0:
                                    print('xxxxxxxxxxxxxxxxxxx 1')
                                    self.ret_neta_estqweb = self.total_ret_estqweb * -1
                    ...
                    print(acum, ' aquiiiiiiiiiiiiiii malditroooo xodjsjdsj -x-x-x-x -s--sd-asd-asd-as-')
                    print(dia_progy)
                    ...
                    print(acum - i.monto, ' acum')
                    print(i.monto)
                    print(formula, ' FORMULA ALC XXXXXXXXXXXXXXXXXXXXXXXX')
...
class Detalleconceptos(models.Model):
    ...
    est_ant = fields.Float(string="Est. Ant", digits=(12, 5))
    est_ant_acum = fields.Float(string="Est. Ant Acum", digits=(12, 5))
...
class OrdenesCambio(models.Model):
    ...
    monto_total_ = fields.Float(string='Monto Total', required=True)
