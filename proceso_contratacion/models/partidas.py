...
class Partidas(models.Model):
    ...
    @api.one
    def imagen_aux(self):
        contadorx = self.env['proceso.galeria'].search_count([("id_partida.id", "=", self.id)])
        if contadorx > 0:
    ...
    id_contrato_relacion = fields.Char(string="ESTA ES LA ID DEL CONTRATO A LA PARTIDA PARA LA RELACION") # VALOR NECESARIO PARA BUSCAR CONTRATO
    # TERMINA IMPORTACION
    ...
    monto_partida = fields.Float(string="Monto", ) # ES EL MONTO DE LA PARTIDA SIN IVA
    iva_partida = fields.Float(string="Iva", )  # compute="iva", store=True
    ...
    def imprimir_accion_excel(self):
        url = "/finiquito_excel/finiquito_excel/?id=" + str(self.id)
        return {"type": "ir.actions.act_url", "url": url, "target": "new", }
    ...
    def porProgramadox(self):
        ...
        print(b_programa.programa_contratos)
        if str(b_programa.programa_contratos) == 'proceso.programa()':
            pass
        else:
            if b_programa.fecha_termino_convenida is not False:
                fecha_termino_pp = b_programa.fecha_termino_convenida
            else:
                fecha_termino_pp = b_programa.fecha_termino_programa

            fecha_termino_contrato = datetime.strptime(str(fecha_termino_pp), date_format)

            acumulado = 0
            cont = 0
            for i in b_programa.programa_contratos:
                cont += 1
                fecha_termino_p = datetime(i.fecha_termino.year, i.fecha_termino.month, 1)
                fechahoy = datetime(fecha_hoy.year, fecha_hoy.month, 1)

                # SI LA FECHA ACTUAL ES MAYOT A LA DEL TERMINO DEL CONTRATO ENTONCES EL PORCENTAJE PROGRAMADO DEBERIA
                # DE SER 100
                if fecha_hoy > fecha_termino_contrato:
                    print('xd')
                    self.porcentajeProgramado = 100.00
                # SI NO, LA FECHA DE HOY ES MENOR O IGUAL A LA DEL TERMINO DEL CONTRATO ENTONCES CALCULAR PORCENTAJE
                elif fecha_hoy <= fecha_termino_contrato:
                    # POSICIONARSE EN EL PROGRAMA CORRESPONDIENTE DE LA FECHA ACTUAL (MISMO MES Y ANO)
                    if str(fecha_termino_p) <= str(fechahoy):
                        # DIAS TRANSCURRIDOS DESDE EL INICIO DEL PROGRAMA ACTUAL HASTA LA FECHA ACTUAL
                        fechainicioprog = datetime.strptime(str(i.fecha_inicio), date_format)
                        fecha_actual = datetime.strptime(str(hoy), date_format)
                        r = fecha_actual - fechainicioprog
                        dias_trans = r.days + 1

                        # DIAS DEL MES DEL PROGRAMA ACTUAL
                        dia_mes_inicio = datetime.strptime(str(i.fecha_inicio), date_format)
                        dia_mes_termino = datetime.strptime(str(i.fecha_termino), date_format)
                        ...
                        self.porcentajeProgramado = importe_diario
                    else:
                        pass
    ...
    def lista_documentos(self):
        # VISTA OBJETIVO
        view = self.env.ref('control_expediente.exp_import')
        return {
            'type': 'ir.actions.act_window',
            'name': 'Control Expediente',
            'res_model': 'control.expediente',
            'view_mode': 'tree',
            'limit': 500,
            'context': {'search_default_p_id': self.id, 'search_default_group_etapa': True},
            'view_id': view.id,
        }'''
