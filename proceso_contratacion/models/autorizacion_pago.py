...
class AutorizacionPago(models.Model):
    ...
    numero_factura = fields.Integer('Numero de factura')
    ...
    def _numero_contrato_est(self):
        if not self.vinculo_estimaciones:
            ...
            search_oficio = self.env['autorizacion_obra.anexo_tecnico'].search([('concepto.id', '=', self.vinculo_estimaciones.obra.obra.id)])
            self.oficio_anexo = search_oficio.id
            self.inversion_anual_aut = search_oficio.total

            self.monto_letra_estimado = num2words(self.estimacion_facturado, lang='es')
            cent = str(round(self.estimacion_facturado, 2))
    ...
    def _numero_contrato(self):
        if not self.numero_contrato:
            ...
            if not search_oficio:
                ...
                self.monto_letra_estimado = num2words(round(self.estimacion_facturado, 2), lang='es')
                cent = str(round(self.estimacion_facturado, 2))
                self.centavos = str(cent)[-2:] + '/100'
        ...
        if self.tipo_pago == 'Pago':
                self.anticipo_texto = "Anticipo No. 01 del Contrato " + self.numero_contrato.nombre_contrato
