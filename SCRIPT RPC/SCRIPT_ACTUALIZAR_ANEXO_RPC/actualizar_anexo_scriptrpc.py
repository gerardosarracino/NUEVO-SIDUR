...
anexo = odoo.env['autorizacion_obra.oficios_de_autorizacion'].search([])

for x in anexo:
    print(' START')
    b_anexo = odoo.env['autorizacion_obra.oficios_de_autorizacion'].browse(x)
    for b in b_anexo:
        fecha = b.fecha_actual
        # fecha_ = datetime.strptime(str(fecha), "%Y-%m-%d").date()
        if not fecha:
            pass
        else:
            year = fecha.year
            ejercicio = odoo.env['registro.ejercicio'].search([('name', '=', str(year))])
            for y in ejercicio:
                ejercicio_ = y
                # nombre_lista = '%s - %s' % (b.name.name, str(b.concepto.descripcion) + ' - ' + str(b.concepto.obra_planeada.municipio.municipio_delegacion))

                b_anexo.write({'ejercicio': ejercicio_,
                                              })
                print('EXITO', ejercicio_)
