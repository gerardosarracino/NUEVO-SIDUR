...
numero_contrato_id = odoo.env['proceso.eventos_licitacion'].search([("numerolicitacion_evento", "=", 'LPO-926006995-028-2020')]) # ("nombre_partida", "=", 'SIDUR-ED-19-184.1839')
...
for x in numero_contrato_id:
    print(' START')
    b_partida = odoo.env['proceso.eventos_licitacion'].browse(x)

    for x in b_partida:
        participante = odoo.env['proceso.participante'].search([("numerolicitacion", "=", 'LPO-926006995-028-2020')])
        for i in participante:

            b_participante = odoo.env['proceso.participante'].browse(i)
            for o in b_participante.contratista_participantes:
                print('xd', )
                if o.name == 'GRUPO BELALCAN CONSTRUCCTORES, S.A. DE C.V':
                    print('xsssd', )

                    '''partidas_data = {'contratista_aclaraciones': [[0, 0, {
                        'licitacion_id': x.numerolicitacion_evento.id,
                        'name': o.id,
                        'nombre_representante': o.nombre_representante,
                        'correo': o.correo,
                    }]]}'''

                    '''partidas_data = {'contratista_propuesta': [[0, 0, {
                        'id_eventos': x.id,
                        'numerolicitacion': x.numerolicitacion_evento.id,
                        'name': o.id,
                    }]]}'''

                    '''partidas_data = {'contratista_fallo': [[0, 0, {
                        'id_eventos': x.id,
                        'numerolicitacion': x.numerolicitacion_evento.id,
                        'name': o.id,
                        'nombre_representante': o.nombre_representante,
                    ...
                    b_propuesta = odoo.env['proceso.contra_propuestas'].search(
                        [('numerolicitacion.id', '=', x.numerolicitacion_evento.id)])

                    for l in b_propuesta:
                        b_p = odoo.env['proceso.contra_propuestas'].browse(l)
                        for t in b_p:
                            if str(t.name.nombre_representante) == str(o.nombre_representante):
                                xd = t.programar_obra_licitacion2.unlink()

                                '''for u in x.numerolicitacion_evento.programar_obra_licitacion:
                                    datos_propuesta = {
                                    ...
                                    # partida_nueva2 = b_partida.write(partidas_data)
                                    print('exito', datos_propuesta )
                            else:
                                pass'''
