...
partidas = odoo.env['control.estimaciones'].search([('obra', '=', 'SIDUR-PF-17-227.987')]) # ('obra', '=', 'SIDUR-ED-20-014.2001'), ('idobra', '=', '1')
...
for x in partidas:
    ...
    try:
        ...
        for b in b_estimaciones.conceptos_partidas:
            importe_ejecutado = b.precio_unitario * b.estimacion
            # b.write({'importe_ejecutado': importe_ejecutado})
            b.write({'num_est': b_estimaciones.idobra, 'numero_estimacion_group': 'Estimación ' + str(b_estimaciones.idobra)})
            print(b_estimaciones.idobra, b_estimaciones.obra, 'EXITO')
    except:
        print('ERROR')

        '''print(' INICIA CICLO')
        # CASO CATEGORIA
        if b.clave_linea and b.categoria and b.concepto and b.related_categoria_padre.id and not b.precio_unitario:
            print(' YA SIRVE PASAR')

        elif b.clave_linea and b.categoria and b.concepto and not b.related_categoria_padre.id and not b.precio_unitario:
            print(' CON CATEGORIA PERO SIN GRUPO')
            categoria = odoo.env['catalogo.categoria'].search(
                [('name', '=', b.categoria.name), ('id_partida.id', '=', b_estimaciones.obra.id)])

            categoria_relatedx2 = ''
            for y in categoria:
                b_cat = odoo.env['catalogo.categoria'].browse(y)
                categoria_relatedx2 = b_cat.id
            b.write({'related_categoria_padre': categoria_relatedx2})

        elif b.clave_linea and b.categoria and b.concepto and b.categoria.parent_id and not b.precio_unitario:
        ...
        # CASO CONCEPTO
            ...
            if b.categoria and b.related_categoria_padre:
                print(' CONCEPTO COMPLETO PASAR')
            elif b.related_categoria_padre.parent_id.id and not b.categoria:
                print(' SI TIENE SUB GRUPO EL CONCEPTO PERO NO CATEGORIA')
