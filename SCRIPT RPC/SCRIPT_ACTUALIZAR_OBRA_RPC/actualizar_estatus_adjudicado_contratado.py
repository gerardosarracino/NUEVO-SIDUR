...
obra = odoo.env['registro.programarobra'].search([]) # ("nombre_partida", "=", 'SIDUR-ED-19-133.1779')

# obra = odoo.env['registro.programarobra'].search([])

for x in obra:
    print(' START')
    b_obra = odoo.env['registro.programarobra'].browse(x)

    for b in b_obra:
        adj = odoo.env['partidas.adjudicacion'].search([('obra.id', '=', b.id)])
        print(adj, 'xd')
        if adj:
            print('SIMONA LA PELONA')
