...
numero_contrato_id = odoo.env['partidas.partidas'].search([("nombre_partida", "=", 'SIDUR-ED-19-133.1779')]) # ("nombre_partida", "=", 'SIDUR-ED-19-133.1779')
...
for x in numero_contrato_id:
    ...
    for b in b_partida:
        ...
        if not lista:
            ...
            b_estados = odoo.env['semaforo.estado_obra'].search([('partidaid.id', '=', b_partida.id)])
            for y in b_estados:
                b_es = odoo.env['semaforo.estado_obra'].browse(y)
                for x in b_es:
                    if str(x.fecha) == str(max(lista)):
                        datos = {'tipo_estado_obra': x.tipo_estado}
