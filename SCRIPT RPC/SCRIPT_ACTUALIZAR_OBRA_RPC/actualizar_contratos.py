...
numero_contrato_id = odoo.env['proceso.elaboracion_contrato'].search([]) # ("nombre_partida", "=", 'SIDUR-ED-19-133.1779')
...
for x in numero_contrato_id:
    ...
    for b in b_partida:
        ...
        print(year)
        ejercicio = 0
        b_eje = odoo.env['registro.ejercicio'].search([])
        for i in b_eje:
            b_ejer = odoo.env['registro.ejercicio'].browse(i)
            for y in b_ejer:
                if str(year) == str(y.name):
                    ejercicio = y.id
                else:
                    pass
        t_obra = 0
        ...
        if b.adjudicacion:
            print('ADJ')
            for p in b.contrato_partida_adjudicacion[0]:
                p_inv = p.programaInversion
                t_obra = p.obra.obra_planeada.tipoObra
        else:
            print('LICI')
            for p in b.contrato_partida_adjudicacion[0]:
                p_inv = p.programaInversion.id
                t_obra = p.obra.obra_planeada.tipoObra.id
        datos = {'ejercicio': ejercicio, 'tipo_obra': t_obra, 'programaInversion': p_inv}
        b_partida.write(datos)
        print(' EXITOOOO', b.contrato)
