...
# numero_contrato_id = odoo.env['partidas.partidas'].search([]) # ("nombre_partida", "=", 'SIDUR-PF-20-008.2005')

_search_cove = odoo.env['proceso.convenios_modificado'].search([])
print('INICIA')
for c in _search_cove:
    convenio = odoo.env['proceso.convenios_modificado'].browse(c)
    for i in convenio:
        convenio.write({'contrato_contrato': i.contrato.numero_contrato.id, 'nombre_contrato': str(i.contrato.nombre_contrato)})
        print('termino', i.contrato.nombre_contrato)
