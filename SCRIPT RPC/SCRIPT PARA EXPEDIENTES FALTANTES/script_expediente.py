...
numero_contrato_id = odoo.env['partidas.partidas'].search([("ejercicio", "=", '2020')]) # ("nombre_partida", "=", 'SIDUR-ED-19-184.1839')
...
for x in numero_contrato_id:
    ...
    for x in b_partida:
        print('xd')
        b_expediente = odoo.env['control_expediente.control_expediente'].search([])
        for y in b_expediente:
            b_exp = odoo.env['control_expediente.control_expediente'].browse(y)
            for o in b_exp:
                tabla_control_list = {'tabla_control': [[0, 0, {
                    'nombre': o.id,
                    'p_id': x.id,
                    'etapa': o.etapa
                }]]}

                partida_nueva2 = b_partida.write(tabla_control_list)
