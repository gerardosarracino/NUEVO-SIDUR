...
informes = odoo.env['proceso.iavance'].search([('numero_contrato', '=', 'SIDUR-PF-17-227.987')]) # ('numero_contrato', '=', 'SIDUR-ED-19-161.1807'),('num_avance', '=', 15)

for x in informes:
    print(' START')
    b_informe = odoo.env['proceso.iavance'].browse(x)
    for b in b_informe:

        '''r_porcentaje_est = 0
        r_avance_fisico = 0
        for i in b.ruta_critica:
            porcentaje_est = i.porcentaje_est
            r_porcentaje_est += porcentaje_est

            avance_fisico = i.avance_fisico_ponderado
            r_avance_fisico += avance_fisico

            resultado = (r_porcentaje_est * r_avance_fisico) / 100

        porcentaje_estimado = float(resultado)

        b_informe.write({'porcentaje_estimado': porcentaje_estimado,})
        print('EXITO', porcentaje_estimado, b.numero_contrato.nombre_partida)'''
        ...
        b_partida = odoo.env['partidas.partidas'].browse(b.numero_contrato.id)
        b_programa = odoo.env['programa.programa_obra'].search([('obra.id', '=', b.numero_contrato.id)])
        ...
        for l in b_programa:
            b_p = odoo.env['programa.programa_obra'].browse(l)

            for u in b_p.programa_contratos:
                ...
                if str(fecha_termino_pp) == 'False':
                    ...
                    cont = 0
                    porcentajeProgramado = 0
                    for i in b_p.programa_contratos:
                        cont += 1
                        ...
                        if fecha_hoy > fecha_termino_contrato:
                            ...
                            if atraso <= 5:
                                ...
                                color = 'Rojo'
                                b_partida.write({'color_semaforo': color})
                            b_informe.write({'porcentajeProgramado': porcentajeProgramado})
        ...
        for t in b_programa:
            b_p = odoo.env['programa.programa_obra'].browse(t)
            for j in b_p:
                if str(j.programa_contratos) == "proceso.programa()":
                    avance_financiero = 0
                else:
                    b_est = odoo.env['control.estimaciones'].search([('obra.id', '=', b.numero_contrato.id)])
                    for x in b_est:
                        b_ests = odoo.env['control.estimaciones'].browse(x)
                        for q in b_ests:
                            acum += q.estimado
                    avance_financiero = (acum / b_p.total_programa) * 100
                datos = {'a_fis': resultado, 'a_fin': avance_financiero, 'porcentajeProgramado': porcentajeProgramado,
                         'atraso': porcentajeProgramado - porcentaje_estimado}  # 'porcentajeProgramado': self.porcentajeProgramado,
