...
class EstimacionesCalculos(models.Model):
    ...
    def estimacion_finiquito_metodo(self):  # suma_conceptos
        estimacion_search = self.env['control.estimaciones'].search([('obra', '=', self.obra.id)])
        if self.si_aplica_estimacion:  # VERFICAR SI APLICA FINIQUITO
            acum = 0
            ret = self.ret_neta_est
            for i in estimacion_search:
                if int(i.idobra) == int(self.idobra) or int(i.idobra) > int(self.idobra):
                    pass
                else:
                    acum += i.ret_neta_est
            self.ret_neta_est = (acum * -1) 
            self.devolucion_est = (acum * -1) 
            if self.tipo_estimacion == '3' or self.tipo_estimacion == '2':
                self.sancion = 0
                self.devolucion_est = 0
                self.ret_neta_est = 0
            self.a_pagar = (self.estimacion_facturado - self.estimado_deducciones - self.sancion) + self.devolucion_est
        else:
