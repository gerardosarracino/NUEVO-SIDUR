    ...
    def index(self, **kw):
        try:
            ...
            workbook = load_workbook(
                filename="/home/gerardo/Developments/odoo12/extra-addons/finiquito_excel/static/plantilla.xlsx")
            ...
            for i in partida:
                ...
                conceptos = http.request.env['proceso.conceptos_part'].search(
                    [('id_partida.id', '=', b_partida.id)])
                for y in conceptos:
                    ...
                    for x in estimacion:
                        ...
                        for j in detalle_concepto:
                            # HEADER ESTIMACIONES
                            sheet[estimacion_header + '10'] = 'CANTIDAD'  # ESTIMACION
                            sheet[estimacion_header + '10'].style = header_est_estilo
...
class ConceptosContratados(http.Controller):
    @http.route('/conceptos_contratados/conceptos_contratados/', auth='public')
    def index(self, **kw):
        try:
            partida = http.request.env['partidas.partidas'].search(
                [('id', '=', kw['id'])])  # , ('idobra', '=', '1')

            workbook = load_workbook(
                filename="/home/gerardo/Developments/odoo12/extra-addons/ejecucion_obra/static/conceptos.xlsx")
            sheet = workbook.active
            wb = Workbook()
            ...
            contratista = ''
            for o in partida:
                nombre_partida = o.nombre_partida
                contratista = o.contratista.name
                acum = 0
                pos_concepto = 0
                aviso = 0
                xd = 0
                for y in o.conceptos_partidas:
                    # idobra = i.idobra
                    # periodo_del = i.fecha_inicio_estimacion
                    # periodo_al = i.fecha_termino_estimacion
                    # for y in estimacionx.conceptos_partidas:
            ...
            # Save the spreadsheet
            workbook.save("/tmp/catalogo_conceptos.xlsx")

            # prs.save('/tmp/test.pptx')
            f = open('/tmp/catalogo_conceptos.xlsx', mode="rb")
            return http.request.make_response(f.read(),
                                              [('Content-Type', 'application/octet-stream'),
                                               ('Content-Disposition',
                                                'attachment; filename="{}"'.format(nombre_partida + '_catalogo_conceptos.xlsx'))
                                               ])
