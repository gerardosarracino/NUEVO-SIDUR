...
query = ("SELECT * FROM `deducciones` INNER JOIN estimaciones_deduciones ON deducciones.id = estimaciones_deduciones.id_deducion WHERE estimaciones_deduciones.num_contrato = 'SIDUR-ED-19-159.1810' and estimaciones_deduciones.num_estimacion = '4' order by estimaciones_deduciones.id asc;" ) #  WHERE id > 15965 where estimaciones_deduciones.id > 20382
...
for row in cursor:
    ...
    _search_esti_count = odoo.env['control.estimaciones'].search_count(
            [("obra", "=", row['num_contrato']), ("idobra","=",row['num_estimacion']),  ("tipo_estimacion","=", row['tipoEstimacion'])])
